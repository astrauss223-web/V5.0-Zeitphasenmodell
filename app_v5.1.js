...
function ensureDelistetFunds() {
    ...
    managementBlocks.forEach(block => {
        if (block.id === 'block-tagesgeld' || block.title.toLowerCase().includes('tagesgeld')) {
...
function deleteFundCompletely(blockId, fundName) {
    removeFund(blockId, fundName);
    if (typeof managementBlocks !== 'undefined') {
        const block = managementBlocks.find(b => b.id === blockId);
        if (block && block.funds) {
            block.funds = block.funds.filter(f => f.name !== fundName);
        }
    }
    if (typeof currentBlockData !== 'undefined' && currentBlockData && currentBlockData.id === blockId) {
        const modal = document.getElementById('funds-modal');
        if (modal && modal.classList.contains('open') && typeof renderFundList === 'function') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createCylinderElement(block) {
        ...
        blockEl.innerHTML = `
            ...
            <div class="layer-assigned-amount ${(sumEinmal > 0 || sumSpar > 0) ? 'visible' : ''}">
                ${sumEinmal > 0 ? `<span class="amount-einmal">${formatCurrency(sumEinmal)}</span>` : ''}
                ${sumSpar > 0 ? `<span class="amount-spar">${formatCurrency(sumSpar)}/mtl.</span>` : ''}
            </div>`;
    ...
    const refreshModalButtons = () => {
        ...
        modalList.querySelectorAll('.fund-list-item').forEach(li => {
            const fName = li.dataset.fundName;
            const sel = isFundSelected(currentBlockData.id, fName);
            ...
            btn.title = sel ? 'Klicken zum Entfernen' : 'Zum Portfolio hinzufügen';
            if (!sel) {
                const inpE = li.querySelector('.fund-investment-input');
                const inpS = li.querySelector('.fund-sparrate-input');
                const key = `${currentBlockData.id}::${fName}`;
                if (inpE && (!portfolioGlobals.fundInvestments[key] || portfolioGlobals.fundInvestments[key] <= 0)) inpE.value = '';
                if (inpS && (!portfolioGlobals.fundSparrates[key] || portfolioGlobals.fundSparrates[key] <= 0)) inpS.value = '';
            }
        });
    };
        ...
        filtered.forEach(fund => {
            ...
            selBtn.addEventListener('click', (e) => {
                ...
                if (isFundSelected(currentBlockData.id, fund.name)) {
                    ...
                    const inpE = item.querySelector('.fund-investment-input');
                    const inpS = item.querySelector('.fund-sparrate-input');
                    if (inpE) inpE.value = '';
                    if (inpS) inpS.value = '';
                } else {
            ...
            if (inpEinmal) {
                inpEinmal.addEventListener('input', e => {
                    ...
                    if (v > 0) {
                        ...
                        if (!portfolioGlobals.fundSparrates[key] || portfolioGlobals.fundSparrates[key] <= 0) {
                            removeFund(currentBlockData.id, fund.name);
                        }
                    }
                    saveGlobals();
            ...
            if (inpSpar) {
                inpSpar.addEventListener('input', e => {
                    ...
                    if (v > 0) {
                        ...
                        if (!portfolioGlobals.fundInvestments[key] || portfolioGlobals.fundInvestments[key] <= 0) {
                            removeFund(currentBlockData.id, fund.name);
                        }
                    }
                    saveGlobals();
    ...
    function updatePortfolioUI() {
        ...
        if (panelLayers) {
            if (!panelLayers.__removeListenerAttached) {
                panelLayers.__removeListenerAttached = true;
                panelLayers.addEventListener('click', (e) => {
                    const removeBtn = e.target.closest('.panel-fund-remove');
                    if (removeBtn) {
                        e.stopPropagation();
                        e.preventDefault();
                        const bId = removeBtn.dataset.block;
                        const fName = removeBtn.dataset.fund;
                        if (bId && fName) {
                            deleteFundCompletely(bId, fName);
                        }
                    }
                });
            }
    ...
    const generateAnlagevorschlagPDF = () => {
        if (!window.jspdf) return;
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        const now = new Date();
        const dateStr = now.toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' });
        const fileName = `Anlagevorschlag_V5.1_${dateStr.replace(/\./g, '-')}.pdf`;

        doc.setFontSize(18); doc.setTextColor(3, 61, 93);
        doc.text('Anlagevorschlag - Robustes Portfolio V5.1', 14, 20);
    ...
    if (savePortfolioBtn) savePortfolioBtn.addEventListener('click', generateAnlagevorschlagPDF);
    if (savePortfolioPdfBtn) savePortfolioPdfBtn.addEventListener('click', generateAnlagevorschlagPDF);
